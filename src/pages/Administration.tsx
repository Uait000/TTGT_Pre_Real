...
import { settingsApi, AdministrationItem } from '@/api/settings';
import adm_1 from '@/assets/pictures/Zavyalov.png';
// ... остальные импорты (оставьте как было)
...
const DEFAULT_STAFF: AdministrationItem[] = [
...
const Administration = () => {
    const [staff, setStaff] = useState<AdministrationItem[]>(DEFAULT_STAFF);

    useEffect(() => {
        const loadData = async () => {
            const data = await settingsApi.getPageSettings('administration_page');
            if (data) {
                const realData = Array.isArray(data) ? data : (data.items || []);
                if (realData.length > 0) setStaff(realData);
            }
        };
        loadData();
    }, []);
        ...
        <MainLayout>
            <div className="bg-white rounded-lg shadow-sm border border-border p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {staff.map((person) => (
                        <div key={person.id} className="bg-gradient-to-br from-primary/5 to-secondary/5 rounded-lg border border-border/50 overflow-hidden hover:shadow-lg hover:-translate-y-2 transition-all duration-300 cursor-pointer">
                            <div className="aspect-[4/5] bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center overflow-hidden">
                                <img src={person.photo} alt={person.name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                            </div>
                            <div className="p-4">
                                <div className="bg-primary/10 rounded-lg p-3 mb-3">
                                    <h3 className="font-semibold text-primary text-center text-sm">{person.position}</h3>
                                </div>
                                <div className="space-y-2 text-sm">
                                    <p className="font-medium text-foreground text-center">{person.name}</p>
                                    <p className="text-muted-foreground"><strong>телефон:</strong> {person.phone}</p>
                                    <p className="text-muted-foreground"><strong>e-mail:</strong> <a href={`mailto:${person.email}`} className="text-blue-600 hover:text-blue-800">{person.email}</a></p>
                                    <div className="pt-2 border-t border-border/50">
                                        <p className="text-xs text-muted-foreground"><strong>ГРАФИК ПРИЕМА ГРАЖДАН:</strong></p>
